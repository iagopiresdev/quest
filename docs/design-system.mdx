...
The repo keeps internal domain names plain and uses RPG flavor only in operator-facing presentation.
...
- `Turn-in`
  This is the landing step from the integration worktree back into the source checkout. Flavor is fine in the UI, but the implementation still needs explicit landing rules and drift checks.
